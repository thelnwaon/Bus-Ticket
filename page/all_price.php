  ...
  <h4>&nbsp;&nbsp;&nbsp;&nbsp;การจองตั๋วเที่ยวรถ</h4>
          ...
          <th width="100px"><center>วันที่จอง</center></th>
          	...
          	require 'config.php';
          	  ...
          	  $sql  ="SELECT route.*,bustype.bustype_name,reservation.*,user.* FROM reservation  INNER JOIN route ON  route.rou_id = reservation.id_rou INNER JOIN bustype ON route.id_bustype = bustype.bustype_id INNER JOIN user ON  reservation.id_user=user.user_id WHERE reservation.status='1'";
                  ...
                  <td><center><?= $rs2['date_gone']," : ",$rs2['arr_time']?></center></td>
                       ...
                       $i ++;}
          	?>
          						<tr><td colspan="10"><center>รวม</center></td><td><center><?= $total?></center></td></tr>
          	</table>
          	</div>
          	</div>
          </div>
