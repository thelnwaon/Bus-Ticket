  ...
  <h4>&nbsp;&nbsp;&nbsp;&nbsp;การจองตั๋วเที่ยวรถ</h4>
          ...
          <th width="100px"><center>วันที่จอง</center></th>
                  ...
                  <th id="hide"><center>พิมพ์ตั๋ว</center></th>
                  </tr>
          	<?php 
          	require 'config.php';

          	  $sql  ="SELECT route.*,bustype.bustype_name,reservation.*,user.* FROM reservation  INNER JOIN route ON  route.rou_id = reservation.id_rou INNER JOIN bustype ON route.id_bustype = bustype.bustype_id INNER JOIN user ON  reservation.id_user=user.user_id WHERE reservation.id_user='".$_SESSION['user_id']."'";
                  ...
                  <td><center><?= $rs2['date_gone']," : ",$rs2['arr_time']?></center></td>
         					...
         					if($rs2['status'] == "0"){
         						?>
         					<span class="label label-warning">ยังไม่ชำระ</span>
         						<?php
         					}else{
         					?>
         					...
         					<td id="hide">
         					<a href="?p=print_booking&id=<?= $rs2['res_id']?>" target="__blank"><button class="btn btn-danger">พิมพ์ตั๋ว</button></a>	
         					</td>
         					</tr>
                      	<?php
                       $i ++;}
